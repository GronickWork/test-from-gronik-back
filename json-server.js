...
app.use(async (ctx, next) =>{ 
  ...
  switch (ctx.request.method) {
    case 'GET':
      ...
      break;
    case 'POST':
      if(!recData.title || !recData.description || !recData.photo || !recData.date || !recData.time) {
        ctx.status = 400;
    ...
    case 'PUT':
      if(!recData.id) {
        ctx.status = 400;
        ctx.error = 'Действие не возможно. Не достаточно данных для данной операции.';
        break;
      }
      ...
      if(photo) {change.photo = photo;}
      if(date) {change.date = date;}
      if(time) {change.time = time;}
